function [wm, bm, acc, map, classifications]=train_test_vlfeat(methods, SVMopts, trainFs, trainY, testFs, testY)
% linear SVM classification wrapper of vlfeat's SVM
    % some hyper parameters
    % SVMopts.epsilon, reg_coeff, maxiter
    reg_coeff=1/size(trainFs, 2);
    if strcmp(methods, 'LR')
        reg_coeff=reg_coeff/1000;
    end
    reg_coeff = reg_coeff * SVMopts.reg_coeff; %1
    epsilon= SVMopts.epsilon; %1E-3
    maxiter = SVMopts.maxiter; %1E6
