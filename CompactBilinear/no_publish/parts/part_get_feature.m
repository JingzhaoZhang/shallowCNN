...
function out=get_one(conv5, parts_loc)
    nparts=15;
    projDim=8192;
    neighbour=2; % is the delta 
    
    [h, w, c, n]=size(conv5);
    assert(h==w, 'Assumption that height==width failed');
    
    out=zeros(projDim*(nparts+1), n, 'single');
    ...
    for iout=1:nbatch:n
        ...
        for i=inter
            ...
            end  % end of part loop
            
            % get a whole image descriptor all the time
            feature=sum(sum(projected(:, :, :, i-iout+1) ,1), 2);
            feature=squeeze(feature);
            feature=l2norm(sq(feature));
            
            j=nparts+1;
            out((projDim*(j-1)+1):projDim*j, i)=feature;
        end % end of inner interval loop 
